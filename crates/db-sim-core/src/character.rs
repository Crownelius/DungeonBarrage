...
const ARZUM_PASSIVES: &[PassiveDefinition] = &[ARZUM_MOMENTUM, ARZUM_CHAIN_REACTION, ARZUM_LIGHTFOOT];
...
const EMI_PASSIVES: &[PassiveDefinition] = &[EMI_REINFORCED_CASING, EMI_DENSE_PAYLOAD, EMI_OVERWATCH];
...
const ROBERTO_PASSIVES: &[PassiveDefinition] = &[ROBERTO_DOUBLE_BOUNCE, ROBERTO_BLAST_SHIELDING, ROBERTO_SHRAPNEL];
...
const NATOMICA_PASSIVES: &[PassiveDefinition] = &[NATOMICA_ANCHORED, NATOMICA_KINETIC, NATOMICA_BULWARK];
...
fn validate_roster_slice(roster: &[CharacterDefinition]) -> SimResult<()> {
    ...
    for character in roster {
        ...
        validate_ability(&mut all_ability_ids, &character.special, AbilitySlot::Special)?;
...
mod tests {
    ...
    fn only_aleph_has_basic_alt() {
        let aleph_alt_count = LAUNCH_ROSTER.iter().filter(|c| c.basic_alt.is_some()).count();
        ...
        for character in LAUNCH_ROSTER.iter().filter(|c| c.id != "aleph") {
            assert!(character.basic_alt.is_none(), "{} unexpectedly has a basic_alt", character.id);
        }
    }
    ...
    fn starter_core_stats_match_characters_md() {
        ...
        assert_eq!(expected.len(), LAUNCH_ROSTER.len(), "test table must cover the whole roster");
    ...
    fn starter_basic_damage_matches_characters_md() {
        ...
        for (id, dmg) in expected {
            ...
            assert_eq!(character.basic.damage_percent, *dmg, "{id} basic damage_percent");
        }
    }
    ...
    fn huck_haymaker_terrain_radius_is_2_bw() {
        ...
        match huck.basic.attack {
            Attack::Strike(StrikeAttack { terrain: TerrainProfile::Crater { radius_cells }, .. }) => {
    ...
    fn huck_body_throw_damage_splits_40_and_25() {
        ...
        let Some(relocate) = huck.special.effects.iter().find(|e| e.kind == EffectKind::Relocate) else {
    ...
    fn zeke_mending_bolt_deals_damage_and_heals() {
        ...
        let Some(heal) = zeke.basic.effects.iter().find(|e| e.kind == EffectKind::Heal) else {
            panic!("Mending Bolt must carry a Heal effect");
        };
        assert_eq!(heal.magnitude, 22);
        assert_eq!(heal.trigger, EffectTrigger::OnImpact, "heal must be tied to the bolt landing");
    }
    ...
    fn arzum_chain_strike_second_hit_ranges_50_to_200_percent() {
        ...
        let Some(teleport) = arzum.special.effects.iter().find(|e| e.kind == EffectKind::Teleport) else {
            panic!("Chain Strike must carry a Teleport effect");
        };
        assert_eq!(teleport.magnitude, 50, "second hit damage range lower bound");
        assert_eq!(teleport.magnitude_secondary, 200, "second hit damage range upper bound");
    }
    ...
    fn emi_wind_response_is_lowest_in_roster() {
        ...
        let emi_wind = match emi.basic.attack {
            Attack::Projectile(ProjectileAttack { wind_scale_basis_points, .. }) => wind_scale_basis_points,
            Attack::Strike(_) => panic!("Emi's basic must be a projectile"),
        };
        assert_eq!(emi_wind, 2_000);

        for character in LAUNCH_ROSTER {
            for ability in [Some(&character.basic), character.basic_alt.as_ref(), Some(&character.special)]
                .into_iter()
                .flatten()
            {
                if let Attack::Projectile(ProjectileAttack { wind_scale_basis_points, .. }) = ability.attack {
                    assert!(
    ...
    fn aleph_throwing_knife_deals_60_percent_and_embeds() {
        ...
        assert!(knife.effects.iter().any(|e| e.kind == EffectKind::EmbedProjectile));
    }
    ...
    fn aleph_dagger_chain_matches_characters_md() {
        ...
        let Some(chain) = knife.effects.iter().find(|e| e.kind == EffectKind::ChainDetonate) else {
            panic!("throwing knife must carry a ChainDetonate effect (CHARACTERS.md's dagger chain)");
        };
        assert_eq!(chain.magnitude, 70, "chain detonation damage percent");
        assert_eq!(chain.magnitude_secondary, 10 * POSITION_SCALE, "2.5 BW detonation/terrain radius");

        let Some(embed) = knife.effects.iter().find(|e| e.kind == EffectKind::EmbedProjectile) else {
            panic!("throwing knife must carry an EmbedProjectile effect");
        };
        assert_eq!(embed.magnitude, 6 * POSITION_SCALE, "1.5 BW chain-trigger proximity");
    ...
    fn natomica_repulse_and_wall_impact_match_characters_md() {
        ...
        let Some(wall_impact) = natomica.special.effects.iter().find(|e| e.kind == EffectKind::WallImpact) else {
    ...
    fn numa_harpoon_threshold_is_50_percent_in_basis_points() {
        ...
        let Some(pull) = numa.basic.effects.iter().find(|e| e.kind == EffectKind::Pull) else {
            panic!("Harpoon must carry a Pull effect");
        };
        assert_eq!(pull.magnitude_secondary, 5_000, "50% in basis points, not 50_000");
    }
    ...
    fn projectile_tiers_can_reach_their_own_range_tier() {
        let cases: &[(i32, i32, i32)] = &[
            (PROJECTILE_TIER1_SPEED, PROJECTILE_TIER1_GRAVITY, RangeTier::Tier1.reach()),
            (PROJECTILE_TIER2_SPEED, PROJECTILE_TIER2_GRAVITY, RangeTier::Tier2.reach()),
            (PROJECTILE_TIER3_SPEED, PROJECTILE_TIER3_GRAVITY, RangeTier::Tier3.reach()),
        ];
    ...
    fn projectile_tiers_max_ticks_exceed_worst_case_hang_time() {
        let cases: &[(i32, i32, u16)] = &[
            (PROJECTILE_TIER1_SPEED, PROJECTILE_TIER1_GRAVITY, PROJECTILE_TIER1_MAX_TICKS),
            (PROJECTILE_TIER2_SPEED, PROJECTILE_TIER2_GRAVITY, PROJECTILE_TIER2_MAX_TICKS),
            (PROJECTILE_TIER3_SPEED, PROJECTILE_TIER3_GRAVITY, PROJECTILE_TIER3_MAX_TICKS),
        ];
    ...
    const DUMMY_PASSIVES: &[PassiveDefinition] = &[DUMMY_PASSIVE_A, DUMMY_PASSIVE_B, DUMMY_PASSIVE_C];
    ...
    fn dummy_character(id: &'static str, basic_id: &'static str, special_id: &'static str) -> CharacterDefinition {
        CharacterDefinition {
            id,
            version: 1,
            display_name: "Dummy",
    ...
    fn dummy_character_without_aleph_fails_only_the_basic_alt_check() {
        ...
        assert_rejected(validate_roster_slice(&[c]), CharacterRejection::StatOutOfRange);
    }
    ...
    fn duplicate_character_id_is_rejected() {
        ...
        assert_rejected(validate_roster_slice(&[a, b]), CharacterRejection::DuplicateId);
    }
    ...
    fn ability_slot_mismatch_is_rejected_with_correct_reason() {
        ...
        assert_rejected(validate_roster_slice(&[c]), CharacterRejection::SlotMismatch);
    }
    ...
    fn special_slot_mismatch_is_rejected_with_correct_reason() {
        let mut c = dummy_character("mismatched-special", "mismatched-special-basic", "mismatched-special-special");
    ...
    fn wrong_passive_count_too_few_is_rejected() {
        ...
        c.passives = TOO_FEW;
        assert_rejected(validate_roster_slice(&[c]), CharacterRejection::WrongPassiveCount);
    }

    #[test]
    fn wrong_passive_count_too_many_is_rejected() {
        const EXTRA: &[PassiveDefinition] =
            &[DUMMY_PASSIVE_A, DUMMY_PASSIVE_B, DUMMY_PASSIVE_C, DUMMY_PASSIVE_D];
    ...
    fn empty_passive_id_is_rejected() {
        const EMPTY_ID: PassiveDefinition =
            PassiveDefinition { id: "", display_name: "Empty", kind: PassiveKind::BonusMaxHealth, magnitude: 1 };
        const WITH_EMPTY: &[PassiveDefinition] = &[EMPTY_ID, DUMMY_PASSIVE_B, DUMMY_PASSIVE_C];
        let mut c = dummy_character("empty-passive", "empty-passive-basic", "empty-passive-special");
    ...
    fn too_many_effects_is_rejected() {
        ...
        const TOO_MANY: &[SpecialEffect] = &[EFFECT, EFFECT, EFFECT, EFFECT, EFFECT, EFFECT, EFFECT];
